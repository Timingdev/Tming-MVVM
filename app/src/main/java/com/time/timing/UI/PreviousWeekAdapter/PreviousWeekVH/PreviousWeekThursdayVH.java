...
import com.time.timing.UI.PreviousWeekAdapter.UserAdapter.PreviousThursdayUserAdapter;
import com.time.timing.UI.PreviousWeekAdapter.UserWeekModel.ThursdayUserModel;
import com.time.timing.databinding.PreviousshiftitemBinding;

import java.util.concurrent.Executor;
import java.util.concurrent.Executors;

public class PreviousWeekThursdayVH extends RecyclerView.ViewHolder {
    ...
    private PreviousThursdayUserAdapter previousThursdayUserAdapter;
    private Executor executor;

    public PreviousWeekThursdayVH(@NonNull PreviousshiftitemBinding binding) {
        super(binding.getRoot());
        this.binding = binding;
        executor = Executors.newSingleThreadExecutor();
        previousThursdayUserAdapter = new PreviousThursdayUserAdapter();
        binding.RecyclerView.setHasFixedSize(true);
    }

    public void SetThurdayUser(String Date, String ShiftName, String UID, Context context) {

        if (Date != null) {
            binding.RecyclerView.setLayoutManager(new GridLayoutManager(context, 2));
            binding.RecyclerView.setAdapter(previousThursdayUserAdapter);
                                        ...
                                        if(!value.isEmpty()){
                                            for(var data : value.getDocumentChanges()){
                                                if(data.getType() == DocumentChange.Type.ADDED || data.getType() == DocumentChange.Type.MODIFIED || data.getType() == DocumentChange.Type.REMOVED){
                                                    previousThursdayUserAdapter.setList(value.toObjects(ThursdayUserModel.class));
                                                    previousThursdayUserAdapter.notifyDataSetChanged();
                                                }
                                            }
                                        }
                                    });
