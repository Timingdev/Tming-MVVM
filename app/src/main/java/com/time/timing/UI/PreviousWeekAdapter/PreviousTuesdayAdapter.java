...
public class PreviousTuesdayAdapter extends RecyclerView.Adapter<PreviousTuesdayViewHolder> {
    ...
    @NonNull
    @Override
    public PreviousTuesdayViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        var l = LayoutInflater.from(parent.getContext());
        var view = PreviousshiftitemBinding.inflate(l, parent, false);
        return new PreviousTuesdayViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull PreviousTuesdayViewHolder holder, int position) {
        holder.binding.ShiftTime.setText(list.get(position).getShift_start_time()+"-"+list.get(position).getShift_end_time());
        holder.SetTuesdayUser(list.get(position).getData(), list.get(position).getShiftName(), list.get(position).getUID(), holder.itemView.getContext());
