...
public class EmployeesFridayScheduleGET {
    private Application application;
    private FirebaseAuth Mauth;
    private CollectionReference ScheduleRequestRef;
    private MutableLiveData<List<EmployeesFridayScheduleModel>> data;
    private Executor executor;


    public EmployeesFridayScheduleGET(Application application){
    ...
    public LiveData<List<EmployeesFridayScheduleModel>> GetRequestFridayEmployeesSchedule(String Number, String Date){
                        ...
                        if(!value.isEmpty()){
                            for (var serverdata : value.getDocumentChanges()){
                                if(serverdata.getType() == DocumentChange.Type.ADDED || serverdata.getType() == DocumentChange.Type.MODIFIED || serverdata.getType() == DocumentChange.Type.REMOVED)
                                    data.setValue(value.toObjects(EmployeesFridayScheduleModel.class));
                            }
                        }else {
                            data.setValue(null);
                        }
