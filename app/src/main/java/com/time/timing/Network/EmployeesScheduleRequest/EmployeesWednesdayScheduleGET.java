...
public class EmployeesWednesdayScheduleGET {
    private Application application;
    private FirebaseAuth Mauth;
    private CollectionReference ScheduleRequestRef;
    private MutableLiveData<List<EmployeesWednesdayScheduleModel>> data;
    private Executor executor;


    public EmployeesWednesdayScheduleGET(Application application){
    ...
    public LiveData<List<EmployeesWednesdayScheduleModel>> GetRequestWednesdayEmployeesSchedule(String Number, String Date){
                            ...
                            if(!value.isEmpty()){
                                for (var serverdata : value.getDocumentChanges()){
                                    if(serverdata.getType() == DocumentChange.Type.ADDED || serverdata.getType() == DocumentChange.Type.MODIFIED || serverdata.getType() == DocumentChange.Type.REMOVED)
                                        data.setValue(value.toObjects(EmployeesWednesdayScheduleModel.class));
                                }
                            }else {
                                data.setValue(null);
                            }
