...
public class TuesdayUserRegisterCount {
    ...
    public TuesdayUserRegisterCount(Application application){
        this.application = application;
        MyScheduleRef = FirebaseFirestore.getInstance().collection(DataManager.MySchedule);
        Mauth = FirebaseAuth.getInstance();
    }

    public LiveData<Integer> GetMyScheduleTuesdayUserCount(String UID, String Date){
                                        ...
                                        for(var id : value.getDocuments()){
                                            ...
                                            q.addSnapshotListener(new EventListener<QuerySnapshot>() {
                                                ...
                                                public void onEvent(@Nullable QuerySnapshot value, @Nullable FirebaseFirestoreException error) {
                                                    ...
                                                    if(!value.isEmpty()){
                                                      //  Toast.SetToast(application, String.valueOf(value.size()));
                                                        count++;
                                                        data.setValue(count);
                                                    }
                                                }
