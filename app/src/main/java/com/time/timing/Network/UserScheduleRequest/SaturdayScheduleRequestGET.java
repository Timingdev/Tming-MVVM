...
public class SaturdayScheduleRequestGET {
    private Application application;
    private MutableLiveData<List<SaturdaySchaduleModel>> data;
    private FirebaseAuth Mauth;
    private CollectionReference RequestForShiftRef;

    public SaturdayScheduleRequestGET(Application application) {
        this.application = application;
        Mauth = FirebaseAuth.getInstance();
        RequestForShiftRef = FirebaseFirestore.getInstance().collection(DataManager.RequestForShift);
    }

    public LiveData<List<SaturdaySchaduleModel>> GetSaturdayRequest(String PhoneNumber, String Date) {
                ...
                if (!value.isEmpty()) {
                    for (var mydata : value.getDocumentChanges()) {
                        if (mydata.getType() == DocumentChange.Type.ADDED || mydata.getType() == DocumentChange.Type.MODIFIED || mydata.getType() == DocumentChange.Type.REMOVED) {
                            data.setValue(value.toObjects(SaturdaySchaduleModel.class));
                        }
                    }
                } else {
                    data.setValue(null);
